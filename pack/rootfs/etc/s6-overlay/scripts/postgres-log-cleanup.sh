...
# than a hardcoded /var/lib/aistack path.

SCRIPT_ROOT=/etc/s6-overlay/scripts
source "$SCRIPT_ROOT/base.sh"
# base.sh exports AISTACK_POSTGRES_CONFIG; fall back to its default path so a
...
AISTACK_POSTGRES_CONFIG="${AISTACK_POSTGRES_CONFIG:-${AISTACK_RUN_DIR:-/run/aistack}/postgresql/.env}"
if [ -f "$AISTACK_POSTGRES_CONFIG" ]; then
