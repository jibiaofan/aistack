"""
Helpers to reconcile aistack's configured port ranges (service, Ray) with the
Linux kernel's ephemeral port range.

When an outbound TCP connection is made, the kernel picks a local source port
from `net.ipv4.ip_local_port_range` (default 32768-60999). aistack's default
`ray_port_range` (41000-41999) and `service_port_range` (40000-40063) fall
inside this window, so a worker-side outbound connection (e.g. aistack-worker
...
The fix is to append the aistack ranges to `ip_local_reserved_ports`. This
...
def ensure_reserved_against_ephemeral(
    ...
    if envs.SKIP_RESERVE_EPHEMERAL_PORTS:
        logger.info(
            ...
            "AISTACK_SKIP_RESERVE_EPHEMERAL_PORTS is set."
        )
        return
    ...
    try:
        ...
        logger.warning(
            "aistack port ranges (%s) overlap the kernel ephemeral port "
    ...
    logger.info(
        "Reserved aistack port ranges (%s) against kernel ephemeral range "
        "%d-%d via %s (now: %s).",
        conflict_desc,
        ephemeral[0],
        ephemeral[1],
