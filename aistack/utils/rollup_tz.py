...
Resolution (``resolve_rollup_tz``):
  1. ``AISTACK_TIMEZONE`` (IANA name, e.g. ``Asia/Shanghai``; the legacy
     ``AISTACK_USAGE_ROLLUP_TIMEZONE`` is still honored as a deprecated alias)
...
def resolve_rollup_tz() -> tzinfo:
    ...
    if using_legacy and not _legacy_tz_warned:
        logger.warning(
            "AISTACK_USAGE_ROLLUP_TIMEZONE is deprecated; use AISTACK_TIMEZONE "
    ...
    if tz_name:
        try:
            ...
            var = (
                "AISTACK_USAGE_ROLLUP_TIMEZONE"
                if using_legacy
                else "AISTACK_TIMEZONE"
            )
