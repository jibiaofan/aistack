...
from aistack.scheduler.calculator import calculate_local_model_weight_size
...
from aistack.schemas.workers import Worker, GPUDevicesStatus, GPUDeviceStatus
from pydantic import BaseModel

from aistack.server.services import ModelFileService
from aistack.utils.hub import get_model_weight_size, get_diffusion_model_weight_size
...
def get_vram_claim_from_model_env(model: Model) -> Optional[int]:
    """
    Get the VRAM claim from model environment variable 'AISTACK_MODEL_VRAM_CLAIM' if set.
    """
    if model.env and 'AISTACK_MODEL_VRAM_CLAIM' in model.env:
        try:
            return int(model.env['AISTACK_MODEL_VRAM_CLAIM'])
        except ValueError:
            logger.warning(
                f"Invalid VRAM claim value for model {model.name}: {model.env['AISTACK_MODEL_VRAM_CLAIM']}"
            )
...
def should_skip_gpu_count_check(model: Model) -> bool:
    """
    Whether to bypass the world-size-vs-selected-gpu-count check (AISTACK_SKIP_GPU_COUNT_CHECK).
    ...
    if not model.env:
        return False
    value = model.env.get("AISTACK_SKIP_GPU_COUNT_CHECK")
    if value is None:
        return False
