...
def ext_auth_plugin(cfg: Config) -> Tuple[str, WasmPluginSpec]:
    resource_name = "aistack-llm-ext-auth"
    registry = get_aistack_higress_registry(cfg=cfg)
    ...
    # Scope ext-auth to aistack's own inference routes by route-name prefix,
    # so unrelated traffic on a shared Higress gateway is never authenticated.
    #
    # aistack model-route ingresses are named ``ai-route-route-<id>.internal``
    # (and ``ai-route-route-<id>.fallback.internal`` for fallbacks), both of
    # which share the ``ai-route-route-`` prefix. Other routes -- the aistack
    ...
    #
    # Higress route names carry a ``<namespace>/`` prefix when the aistack
...
def model_pre_route_plugin(cfg: Config) -> Tuple[str, WasmPluginSpec]:
    ...
    expected_spec = WasmPluginSpec(
        ...
        priority=90,
        url=get_plugin_url_with_name_and_version(
            name="aistack-set-header-pre-route", version="1.0.0", cfg=cfg
        ),
    )
    return resource_name, expected_spec
...
def model_mapper_plugin(cfg: Config) -> Tuple[str, WasmPluginSpec]:
    return aistack_model_mapper_name, WasmPluginSpec(
        ...
        priority=800,
        url=get_plugin_url_with_name_and_version(
            name="aistack-model-mapper", version="1.0.0", cfg=cfg
        ),
        defaultConfigDisable=False,
...
def generic_proxy_router_plugin(cfg: Config) -> Tuple[str, WasmPluginSpec]:
    ...
    expected_spec = WasmPluginSpec(
        ...
        priority=900,
        url=get_plugin_url_with_name_and_version(
            name="aistack-generic-proxy-router", version="1.0.0", cfg=cfg
        ),
    )
    return resource_name, expected_spec
...
def token_usage_plugin(cfg: Config) -> Tuple[str, WasmPluginSpec]:
    ...
    expected_spec = WasmPluginSpec(
        ...
        priority=400,
        url=get_plugin_url_with_name_and_version(
            name="aistack-token-usage", version="1.1.0", cfg=cfg
        ),
    )
    return resource_name, expected_spec
