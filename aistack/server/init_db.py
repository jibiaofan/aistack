...
from aistack.server import db
from aistack.utils.db import is_opengauss
...
def _maybe_trace_sql(statement: str):
    """When AISTACK_DB_TRACE_SQL_SUBSTR matches ``statement``, log the Python
    ...
    frames = [
        f
        for f in stack
        if "/aistack/" in f.filename and "server/init_db.py" not in f.filename
    ][-8:]
