...
from aistack.cmd.start import get_aistack_env
from aistack.config.config import Config
from aistack.schemas.users import UserUpdate
from aistack.security import generate_secure_password
    ...
    try:
        ...
        if args.server_url is not None:
            ...
            if server_url is None:
                raise Exception(
                    "Cannot connect to local aistack server. Please specify --server-url"
                )
...
def _resolve_jwt_secret(args) -> str:
    ...
    if not secret:
        ...
        raise Exception(
            "Cannot authenticate to the local aistack server. "
            f"No JWT secret found (checked --jwt-secret-key, "
            f"AISTACK_JWT_SECRET_KEY, and {jwt_secret_path}). "
            "Please provide --api-key, --jwt-secret-key, or "
            "AISTACK_JWT_SECRET_KEY; alternatively run this command on the "
