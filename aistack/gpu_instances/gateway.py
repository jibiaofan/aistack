...
from aistack.schemas.clusters import Cluster, ClusterProvider, K8sOptions
...
async def reconcile_aistack_operator_subscription():
    ...
    async for event in Cluster.subscribe(source="aistack_operator_subscription"):
        if event.type == EventType.HEARTBEAT:
            continue
        try:
            await _reconcile(event)
        except Exception as e:
            logger.error(f"Failed to reconcile aistack-operator subscription: {e}")
