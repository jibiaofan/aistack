...
from aistack.schemas.stmt import model_user_after_drop_view_stmt
...
def upgrade() -> None:
    ...
    ### "aistack-system-{hashed_suffix}" via the now-removed cluster_suffix
    ### template field. After unifying on "aistack-system" new clusters render
    ...
    for cluster_id, hashed_suffix, worker_config, k8s_options in k8s_cluster_rows:
        ...
        # through to the aistack-system default — but still strip any stale
        ...
        target_ns = existing_ns or (
            f'aistack-system-{hashed_suffix}' if hashed_suffix else None
        )
    ...
    with op.batch_alter_table('api_keys', schema=None) as batch_op:
        ...
        if not column_exists('api_keys', 'scope'):
            batch_op.add_column(
                sa.Column('scope', aistack.schemas.common.JSON(), nullable=True)
            )
    ...
    with op.batch_alter_table('api_keys', schema=None) as batch_op:
        batch_op.alter_column(
            'scope',
            existing_type=aistack.schemas.common.JSON(),
            nullable=False,
        )
        batch_op.alter_column(
            'is_custom',
...
def _upgrade_lora() -> None:
    ...
    with op.batch_alter_table("models", schema=None) as batch_op:
        if not column_exists("models", "lora_list"):
            batch_op.add_column(
                sa.Column("lora_list", aistack.schemas.common.JSON(), nullable=True)
            )
    ...
    with op.batch_alter_table("model_instances", schema=None) as batch_op:
        if not column_exists("model_instances", "mounted_loras"):
            batch_op.add_column(
                sa.Column(
                    "mounted_loras", aistack.schemas.common.JSON(), nullable=True
                )
            )
