...
from aistack.schemas.models import BackendEnum
...
class BackendDependencyManager:
    ...
    Examples:
        - model_env: {"AISTACK_BACKEND_DEPS"="transformers==4.53.3,torch>=2.0.0"}
    """
    ...
    def _load_from_environment(self, model_env: Dict[str, str] = None):
        ...
        AISTACK_BACKEND_DEPS="dep1,dep2"
        """
        if not model_env:
            return
        # First try to get from model_env, then fallback to system environment
        env_deps = model_env.get("AISTACK_BACKEND_DEPS")
        ...
        try:
            ...
            logger.warning(f"Failed to parse AISTACK_BACKEND_DEPS: {e}")
