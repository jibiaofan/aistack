...
class ModelRouteTargetClient:
    def __init__(self, client: HTTPClient, enable_cache: bool = True):
        self._client = client
        self._url = "/model-route-targets"
        self._enable_cache = enable_cache
        self._cache: Dict[int, ModelRouteTargetPublic] = {}
    ...
    def list(
        ...
        return ModelRouteTargetsPublic.model_validate(response.json())
    ...
    def _list_from_cache(
        ...
        return ModelRouteTargetsPublic(
        ...
        try:
            ...
            if is_id_only_delete:
                ...
                if item is not None:
                    ...
                    event.data = item
                logger.debug(f"Cache: removed modelroutetarget {event.id}")
                return

            item = ModelRouteTargetPublic.model_validate(event.data)
            ...
            with self._cache_lock:
                if event.type == EventType.DELETED:
                    ...
                    logger.debug(f"Cache: removed modelroutetarget {item.id}")
                else:  # CREATED or UPDATED
                    self._cache[item.id] = item
                    logger.trace(f"Cache: updated modelroutetarget {item.id}")
        except Exception as e:
            logger.error(f"Failed to update model-route-targets cache from event: {e}")
    ...
    def update(self, id: int, model_update: ModelRouteTargetUpdate):
