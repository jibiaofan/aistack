...
class InferenceBackendClient:
    def __init__(self, client: HTTPClient, enable_cache: bool = True):
        ...
        self._cache: Dict[int, InferenceBackendPublic] = {}
    ...
    def list(
        ...
        return InferenceBackendsPublic.model_validate(response.json())
        ...
        try:
            ...
            if is_id_only_delete:
                ...
                if item is not None:
                    ...
                    event.data = item
                logger.debug(f"Cache: removed inferencebackend {event.id}")
                return

            item = InferenceBackendPublic.model_validate(event.data)
            ...
            with self._cache_lock:
                if event.type == EventType.DELETED:
                    ...
                    logger.debug(f"Cache: removed inferencebackend {item.id}")
                else:  # CREATED or UPDATED
                    self._cache[item.id] = item
                    logger.trace(f"Cache: updated inferencebackend {item.id}")
        except Exception as e:
            logger.error(f"Failed to update inference-backends cache from event: {e}")
    ...
    def update(self, id: int, model_update: InferenceBackendUpdate):
