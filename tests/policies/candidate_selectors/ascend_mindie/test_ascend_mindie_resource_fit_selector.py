...
from aistack.policies.candidate_selectors import AscendMindIEResourceFitSelector
...
async def test_select_candidates_3x_64gx8(config, m, expected):
    ...
    with (
        ...
        patch(
            'aistack.policies.candidate_selectors.base_candidate_selector.get_worker_model_instances',
            return_value=model_instances,
        ),
        patch(
            "aistack.schemas.workers.Worker.all",
...
async def test_select_candidates_4x_64gx8(config, m, expected):
    ...
    with (
        patch(
            ...
            new=AsyncMock(return_value=_MOCK_PRETRAINED_DEEPSEEK_R1_0528),
        ),
        patch(
            "aistack.policies.utils.get_worker_model_instances",
            return_value=model_instances,
...
# Mirrors the HuggingFace config of deepseek-ai/DeepSeek-R1-0528 so the
# pretrained-config path doesn't hit ModelScope in CI (the real download can
# exceed the 15s timeout in get_pretrained_config_with_workers and flake).
_MOCK_PRETRAINED_DEEPSEEK_R1_0528 = SimpleNamespace(
...
async def test_lora_vram_estimation(config, case_name, lora_list):
    ...
    with (
        ...
        patch(
            "aistack.policies.candidate_selectors.ascend_mindie_resource_fit_selector.get_model_weight_size",
