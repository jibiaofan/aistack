...
from aistack.config.config import get_openid_configuration
...
def test_get_openid_configuration_uses_system_trust_store(monkeypatch):
    ...
    monkeypatch.setattr("aistack.config.config.httpx.Client", _FakeClient)
    monkeypatch.setattr(
        "aistack.config.config.use_proxy_env_for_url", lambda url: False
    )
