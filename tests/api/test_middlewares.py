...
from aistack.api import middlewares
...
def _stub_async_session(monkeypatch):
    monkeypatch.setattr("aistack.server.db.async_session", lambda: _FakeSessionCtx())
...
def _stub_tenant_context(monkeypatch, *, result=None, exc=None):
    ...
    monkeypatch.setattr("aistack.api.tenant.resolve_tenant_context", _fake)
