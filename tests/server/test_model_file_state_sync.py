...
def _patched(instance: ModelInstance):
    ...
    patches = [
        ...
        patch("aistack.server.controllers.ModelInstanceService", service),
        patch(
            "aistack.server.controllers._build_mounted_loras_payload",
            AsyncMock(return_value=([], [])),
        ),
        patch("aistack.server.controllers.flag_modified", MagicMock()),
        # logger.trace is a custom level not registered in the test runtime.
        patch("aistack.server.controllers.logger", MagicMock()),
    ]
