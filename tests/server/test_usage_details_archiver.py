...
from aistack.server.usage_details_archiver import (
...
def test_archiver_rejects_invalid_cron(monkeypatch):
    monkeypatch.setattr("aistack.envs.USAGE_DETAILS_ARCHIVE_CRON", "garbage")
    with pytest.raises(ValueError, match="USAGE_DETAILS_ARCHIVE_CRON"):
        UsageDetailsArchiver()


def test_archiver_rejects_empty_cron(monkeypatch):
    monkeypatch.setattr("aistack.envs.USAGE_DETAILS_ARCHIVE_CRON", "")
    with pytest.raises(ValueError, match="USAGE_DETAILS_ARCHIVE_CRON"):
        UsageDetailsArchiver()


def test_archiver_reads_retention_and_batch_size(monkeypatch):
    monkeypatch.setattr("aistack.envs.USAGE_DETAILS_RETENTION_MONTHS", 7)
    monkeypatch.setattr("aistack.envs.USAGE_DETAILS_ARCHIVE_BATCH_SIZE", 250)
...
def test_seconds_until_next_fire_within_bound(monkeypatch, expr, upper_bound_seconds):
    monkeypatch.setattr("aistack.envs.USAGE_DETAILS_ARCHIVE_CRON", expr)
...
def test_seconds_until_next_fire_returns_float(monkeypatch):
    monkeypatch.setattr("aistack.envs.USAGE_DETAILS_ARCHIVE_CRON", "*/5 * * * *")
