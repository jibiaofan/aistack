...
def pin_rollup_tz(monkeypatch):
    ...
    monkeypatch.setattr("aistack.server.metrics_collector._ROLLUP_TZ", ZoneInfo("UTC"))
...
def test_estimate_partial_usage_backfills_blank_tokens(monkeypatch):
    ...
    monkeypatch.setattr("aistack.envs.USAGE_ESTIMATED_BYTES_PER_INPUT_TOKEN", 4)
    monkeypatch.setattr("aistack.envs.USAGE_ESTIMATED_TOKENS_PER_OUTPUT_CHUNK", 2)
    metric = ModelUsageMetrics(
        model="m",
        completed=False,
...
def test_estimate_partial_usage_clamps_input_token_to_at_least_one(monkeypatch):
    ...
    monkeypatch.setattr("aistack.envs.USAGE_ESTIMATED_BYTES_PER_INPUT_TOKEN", 100)
    metric = ModelUsageMetrics(
        model="m",
        completed=False,
...
def test_accumulate_caps_details_buffer_and_drops_oldest(monkeypatch, caplog):
    ...
    monkeypatch.setattr("aistack.envs.USAGE_DETAILS_BUFFER_MAX_SIZE", 3)
...
def test_resolve_metric_datetime_buckets_in_local_tz(monkeypatch):
    ...
    # stamps don't drift.
    monkeypatch.setattr(
        "aistack.server.metrics_collector._ROLLUP_TZ", ZoneInfo("Asia/Shanghai")
    )
...
def test_store_usage_metrics_loads_lora_base_model_by_id(monkeypatch):
    ...
    monkeypatch.setattr(
        "aistack.server.metrics_collector.async_session",
        lambda: _FakeSession(),
    )
    ...
    for class_path in (
        "aistack.server.metrics_collector.ModelProvider.all_by_fields",
        "aistack.server.metrics_collector.Principal.all_by_fields",
        "aistack.server.metrics_collector.ApiKey.all_by_fields",
        "aistack.server.metrics_collector.ModelRoute.all_by_fields",
        "aistack.server.metrics_collector.Cluster.all_by_fields",
    ):
