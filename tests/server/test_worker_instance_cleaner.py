...
async def test_cleanup_offline_worker_instances_deletes_main_worker_instances():
    ...
    with (
        patch(
            "aistack.server.worker_instance_cleaner.async_session",
            return_value=mock_async_session(),
        ),
        patch(
            "aistack.server.worker_instance_cleaner.Worker.all",
            AsyncMock(return_value=[offline_worker, other_worker]),
        ),
        patch(
            "aistack.server.worker_instance_cleaner.ModelInstance.all",
            AsyncMock(return_value=[instance]),
        ),
        patch(
            "aistack.server.worker_instance_cleaner.ModelInstanceService.batch_delete",
            batch_delete,
        ),
        patch(
            "aistack.server.worker_instance_cleaner.envs.MODEL_INSTANCE_RESCHEDULE_GRACE_PERIOD",
            300,
        ),
    ):
