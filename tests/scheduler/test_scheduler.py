...
from aistack.policies.utils import should_skip_gpu_count_check
from aistack.scheduler.evaluator import evaluate_model_metadata
from tests.utils.model import make_model, new_model
from aistack.scheduler.scheduler import (
    evaluate_pretrained_config,
    set_model_gpus_per_replica,
)
from aistack.schemas.models import CategoryEnum, BackendEnum
    ...
    [
        ...
        (
            ...
            new_model(
                ...
                backend=BackendEnum.VLLM,
                backend_parameters=[],
                env={"AISTACK_SKIP_MODEL_EVALUATION": "1"},
            ),
            True,
            [],
        ),
...
# --- issue #5089: AISTACK_SKIP_GPU_COUNT_CHECK bypass ---------------------
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            False,
            {"AISTACK_SKIP_GPU_COUNT_CHECK": "1"},
            False,
        ),
        ("manual_no_env", True, None, False),
        ("manual_env_1", True, {"AISTACK_SKIP_GPU_COUNT_CHECK": "1"}, True),
        ("manual_env_true", True, {"AISTACK_SKIP_GPU_COUNT_CHECK": "TRUE"}, True),
        ("manual_env_yes", True, {"AISTACK_SKIP_GPU_COUNT_CHECK": "yes"}, True),
        ("manual_env_on", True, {"AISTACK_SKIP_GPU_COUNT_CHECK": "on"}, True),
        ("manual_env_0", True, {"AISTACK_SKIP_GPU_COUNT_CHECK": "0"}, False),
        ("manual_env_false", True, {"AISTACK_SKIP_GPU_COUNT_CHECK": "false"}, False),
        ("manual_env_empty", True, {"AISTACK_SKIP_GPU_COUNT_CHECK": ""}, False),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("skip", {"AISTACK_SKIP_GPU_COUNT_CHECK": "1"}, 8),
    ],
)
...
def test_set_gpu_count_mismatch_bypassed():
    model = make_model(
        ...
        gpu_ids=_MANUAL_8_GPU_IDS,
        backend=BackendEnum.VLLM.value,
        env={"AISTACK_SKIP_GPU_COUNT_CHECK": "1"},
    )
    selector = _build_selector_for_set_gpu_count(model)
    selector._set_gpu_count(world_size=16, strategies=["tp", "dp"])
