...
async def test_scheduler_subscribes_only_to_created_and_skips_replay():
    ...
    with (
        patch(
            "aistack.scheduler.scheduler.ModelInstance.subscribe",
        ...
        patch("aistack.scheduler.scheduler.AsyncIOScheduler"),
    ):
...
async def test_scheduler_handles_live_created_event_with_single_instance_path():
    ...
    with (
        ...
        with patch("aistack.scheduler.scheduler.AsyncIOScheduler"):
            task = asyncio.create_task(scheduler.start())
            for _ in range(100):
                await asyncio.sleep(0)
