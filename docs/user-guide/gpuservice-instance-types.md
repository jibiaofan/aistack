...
Unlike the other GPU Service resources, instance types are not created by hand. The [AIStack Operator](https://github.com/aistack/aistack-operator) discovers the hardware of every node in a cluster and generates the instance types automatically. They appear as the cards in the leftmost column of the *Add GPU Instance* form.
...
aistack--generic-ln-x64-12c-46g--nvidia-tesla-t4-1d   3/3           36/36   140Gi/140Gi   83Gi/83Gi       Active   5h53m
aistack--generic-ln-x64-1c-2g                         0/0           48/71   96Gi/142Gi    98Gi/377Gi      Active   5h53m
aistack--generic-ln-x64-4c-16g--nvidia-a10g-1d        0/0           0/0     0/0           0/83Gi          Active   5h53m
aistack--generic-ln-x64-4c-16g--nvidia-tesla-t4-1d    1/1           4/4     16Gi/16Gi     98Gi/98Gi       Active   5h53m
```
...
The Operator publishes each node's capacity labels in a `NodeFeature` resource named `<node-name>-aistack-worker` in the `aistack-system` namespace; [Node Feature Discovery](https://github.com/kubernetes-sigs/node-feature-discovery) mirrors them onto the node, and the scheduling chain reads them from there. This `NodeFeature` is also where you tune a node (see [Tuning Instance Types](#tuning-instance-types)).

Inspect the labels of a node — for example, the node with four Tesla T4 devices:

```bash
kubectl -n aistack-system get nodefeature ip-172-31-2-137.ec2.internal-aistack-worker \
  -o jsonpath='{.spec.labels}' | jq -S .
```

```json
{
  "acceleratable.feature.aistack.ai/nvidia-tesla-t4.cpu": "48",
  "acceleratable.feature.aistack.ai/nvidia-tesla-t4.ram": "186Gi",
  "acceleratable.feature.aistack.ai/nvidia-tesla-t4.storage": "98Gi",
  "acceleratable.feature.aistack.ai/nvidia-tesla-t4.z-cohort": "12c-46g-1d",
  "acceleratable.feature.aistack.ai/nvidia-tesla-t4.z-flavor": "48c-186g-98g-4d",
  "acceleratable.feature.aistack.ai/nvidia-tesla-t4.z-queue": "12c-46g-1d",
  "general.feature.aistack.ai/generic": "true",
  "general.feature.aistack.ai/generic-ln-x64": "true",
  "general.feature.aistack.ai/generic-ln-x64.cpu": "48",
  "general.feature.aistack.ai/generic-ln-x64.ram": "96Gi",
  "general.feature.aistack.ai/generic-ln-x64.storage": "98Gi",
  "general.feature.aistack.ai/generic-ln-x64.z-cohort": "1c-2g",
  "general.feature.aistack.ai/generic-ln-x64.z-flavor": "48c-96g-98g",
  "general.feature.aistack.ai/generic-ln-x64.z-queue": "1c-2g",
  "aistack.ai/managed": "true"
}
```

This node exposes both a general view (`generic-ln-x64`, claiming the full 48 CPU / 96 Gi) and a Tesla T4 view (`nvidia-tesla-t4`, claiming the full 48 CPU / 186 Gi across its 4 devices) — the over-subscription described above. The labels you edit are `aistack.ai/managed` and each view's `.cpu`, `.ram`, and `.storage`. The `.z-flavor`, `.z-queue`, `.z-cohort`, and `=true` marker labels are derived by the Operator — leave them alone.

## Tuning Instance Types

You tune instance types by editing the `spec.labels` of a node's `<node-name>-aistack-worker` `NodeFeature`. The Operator reads your `aistack.ai/managed`, `.cpu`, `.ram`, and `.storage` values, recomputes the affected instance types, and re-publishes them.
...
!!! warning
    ...
    Changing a node's labels reshapes or removes the instance types it backs. When an instance type is removed (or drained to zero capacity), AIStack evicts the GPU Service Instances running under it and marks them `Stopped`. Stopped instances are **not** restarted automatically — you start them again from the [GPU Service Instances](gpuservice-instances.md) page once a matching instance type is available. An instance's `Type` itself cannot be changed yet; reconfiguring the `Type` of a Stopped instance is planned for a future release. Apply these changes during a maintenance window.

### Exclude a Node

Set `aistack.ai/managed` to `false` to take a node out of GPU Service entirely. The Operator stops generating instance types from it, so no new instances are scheduled there, and any instance already running on the node is stopped.

```bash
kubectl -n aistack-system patch nodefeature ip-172-31-2-89.ec2.internal-aistack-worker \
  --type merge -p '{"spec":{"labels":{"aistack.ai/managed":"false"}}}'
```
...
```
NAME                                                   ACCELERATOR   CPU     RAM           LOCAL-STORAGE   PHASE    AGE
aistack--generic-ln-x64-12c-46g--nvidia-tesla-t4-1d   3/3           36/36   140Gi/140Gi   83Gi/83Gi       Active   6h15m
aistack--generic-ln-x64-1c-2g                         0/0           48/67   96Gi/134Gi    98Gi/279Gi      Active   6h15m
aistack--generic-ln-x64-4c-16g--nvidia-a10g-1d        0/0           0/0     0/0           0/83Gi          Active   6h15m
aistack--generic-ln-x64-4c-16g--nvidia-tesla-t4-1d    0/0           0/0     0/0           0/0             Active   6h15m
```
...
```bash
kubectl -n aistack-system patch nodefeature ip-172-31-2-89.ec2.internal-aistack-worker \
  --type merge -p '{"spec":{"labels":{"aistack.ai/managed":"true"}}}'
```
...
```bash
kubectl -n aistack-system patch nodefeature ip-172-31-2-137.ec2.internal-aistack-worker \
  --type merge -p '{"spec":{"labels":{"general.feature.aistack.ai/generic-ln-x64.cpu":"0"}}}'
```
...
```
NAME                                                   ACCELERATOR   CPU     RAM           LOCAL-STORAGE   PHASE    AGE
aistack--generic-ln-x64-12c-46g--nvidia-tesla-t4-1d   3/3           36/36   140Gi/140Gi   83Gi/83Gi       Active   6h32m
aistack--generic-ln-x64-1c-2g                         0/0           15/23   30Gi/46Gi     83Gi/279Gi      Active   6h32m
aistack--generic-ln-x64-4c-16g--nvidia-a10g-1d        0/0           0/0     0/0           0/83Gi          Active   6h32m
aistack--generic-ln-x64-4c-16g--nvidia-tesla-t4-1d    1/1           4/4     16Gi/16Gi     98Gi/98Gi       Active   6h32m
```
...
```bash
kubectl -n aistack-system patch nodefeature ip-172-31-2-137.ec2.internal-aistack-worker \
  --type merge -p '{"spec":{"labels":{"general.feature.aistack.ai/generic-ln-x64.cpu":null}}}'
```
...
```bash
kubectl -n aistack-system patch nodefeature ip-172-31-2-137.ec2.internal-aistack-worker --type merge -p '{"spec":{"labels":{
  "general.feature.aistack.ai/generic-ln-x64.cpu":"8",
  "general.feature.aistack.ai/generic-ln-x64.ram":"16Gi",
  "general.feature.aistack.ai/generic-ln-x64.storage":"40Gi",
  "acceleratable.feature.aistack.ai/nvidia-tesla-t4.cpu":"40",
  "acceleratable.feature.aistack.ai/nvidia-tesla-t4.ram":"160Gi",
  "acceleratable.feature.aistack.ai/nvidia-tesla-t4.storage":"56Gi"
}}}'
```
...
```
NAME                                                   ACCELERATOR   CPU     RAM           LOCAL-STORAGE   PHASE    AGE
aistack--generic-ln-x64-10c-40g--nvidia-tesla-t4-1d   4/4           40/40   160Gi/160Gi   56Gi/56Gi       Active   24s
aistack--generic-ln-x64-1c-2g                         0/0           15/31   30Gi/62Gi     83Gi/319Gi      Active   6h37m
aistack--generic-ln-x64-4c-16g--nvidia-a10g-1d        0/0           0/0     0/0           0/83Gi          Active   6h37m
aistack--generic-ln-x64-4c-16g--nvidia-tesla-t4-1d    1/1           4/4     16Gi/16Gi     98Gi/98Gi       Active   6h37m
```
...
aistack-default   jupyter   aistack--generic-ln-x64-12c-46g--nvidia-tesla-t4-1d                                  Stopped
aistack-default   pytorch   aistack--generic-ln-x64-4c-16g--nvidia-a10g-1d        34.230.88.63    22:31025/TCP   Ready
aistack-default   ubuntu    aistack--generic-ln-x64-1c-2g                         100.58.100.48   22:30975/TCP   Ready
```
...
```bash
kubectl -n aistack-system patch nodefeature ip-172-31-2-137.ec2.internal-aistack-worker --type merge -p '{"spec":{"labels":{
  "general.feature.aistack.ai/generic-ln-x64.cpu":null,
  "general.feature.aistack.ai/generic-ln-x64.ram":null,
  "general.feature.aistack.ai/generic-ln-x64.storage":null,
  "acceleratable.feature.aistack.ai/nvidia-tesla-t4.cpu":null,
  "acceleratable.feature.aistack.ai/nvidia-tesla-t4.ram":null,
  "acceleratable.feature.aistack.ai/nvidia-tesla-t4.storage":null
}}}'
```
...
```bash
kubectl -n aistack-system patch nodefeature ip-172-31-2-89.ec2.internal-aistack-worker \
  --type merge -p '{"spec":{"labels":{"acceleratable.feature.aistack.ai/nvidia-tesla-t4.cpu":"0"}}}'
```
...
```
NAME                                                   ACCELERATOR   CPU     RAM           LOCAL-STORAGE   PHASE    AGE
aistack--generic-ln-x64-12c-46g--nvidia-tesla-t4-1d   4/4           48/48   186Gi/186Gi   98Gi/98Gi       Active   2m24s
aistack--generic-ln-x64-1c-2g                         0/0           48/71   96Gi/142Gi    98Gi/377Gi      Active   6h53m
aistack--generic-ln-x64-4c-16g--nvidia-a10g-1d        0/0           0/0     0/0           0/83Gi          Active   6h53m
aistack--generic-ln-x64-4c-16g--nvidia-tesla-t4-1d    0/0           0/0     0/0           0/0             Active   6h53m
```
...
```bash
kubectl -n aistack-system patch nodefeature ip-172-31-2-89.ec2.internal-aistack-worker \
  --type merge -p '{"spec":{"labels":{"acceleratable.feature.aistack.ai/nvidia-tesla-t4.cpu":null}}}'
```
